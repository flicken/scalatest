...
trait Configuration {
   ...
   * @param maxDiscardedDeprecatedUseMaxDiscardedFactorInstead deprecated property for backwards compatibility, use <code>maxDiscardedFactor</code> instead
  ...
  case class PropertyCheckConfig(
    ...
    maxDiscardedDeprecatedUseMaxDiscardedFactorInstead: Int = -1,
    minSize: Int = 0,
    maxSize: Int = 100,
    workers: Int = 1,
    ...
    if (maxDiscardedDeprecatedUseMaxDiscardedFactorInstead != -1) {
      require(maxDiscardedDeprecatedUseMaxDiscardedFactorInstead >= 0, "maxDiscardedDeprecatedUseMaxDiscardedFactorInstead had value " + maxDiscardedDeprecatedUseMaxDiscardedFactorInstead + ", but must be greater than or equal to zero")
    }
    ...
    @deprecated def maxDiscarded = minSuccessful * maxDiscardedFactor

    def calculatedMaxDiscardedFactor = if (maxDiscardedDeprecatedUseMaxDiscardedFactorInstead == -1) {
      maxDiscardedFactor
    } else {
      ((maxDiscardedDeprecatedUseMaxDiscardedFactorInstead + 1): Float) / (minSuccessful: Float)
    }
  }
