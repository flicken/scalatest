...
trait Configuration {
  ...
  private[prop] def getParams(
    ...
    new Parameters {
      ...
      val maxSize: Int = {
        (psizeRange, pmaxSize, config.legacyMaxSize, pminSize) match {
          case (None, None, Some(legacyMaxSize), Some(specifiedMinSize)) =>
            config.legacyMaxSize.get
          case (None, Some(maxSize), _, _) =>
            maxSize
          case _ =>
